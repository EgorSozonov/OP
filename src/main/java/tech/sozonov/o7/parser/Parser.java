    ...
    while (backtrack.peek() != null) {
        ...
        while (i < currInput.val.size()) {
            ...
            if (сurrToken instanceof ListExpr le2) {
                ...
                // [] - nest
                if (curr.isUnbounded()) {
                    // accepts only statements, and only acceptable ones
                } else {
                    //
                    // if core syntax, accepts curly, paren, statements
                    // if unbounded-able, then checks if statement is acceptable
                }
                ...
                if (le2.pType == ExprLexicalType.curlyBraces) {
                    ...
                    val listType = determineListType(le2, syntax.syntax);
